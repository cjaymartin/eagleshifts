...
import { z } from 'zod';
import { useBusinessNotificationSettingsQuery, useUpdateBusinessNotificationSettingsMutation } from '@/queries/team';
...
export default function BusinessNotificationsForm() {
  const { data: notificationSettings, isLoading } = useBusinessNotificationSettingsQuery();
  const updateNotificationsMutation = useUpdateBusinessNotificationSettingsMutation();
  ...
  React.useEffect(() => {
    if (notificationSettings) {
      reset({
        digestEmailDate: notificationSettings.digestEmailDate || 'saturday',
      });
    }
