...
import { z } from 'zod';
import { useBusinessShiftSettingsQuery, useUpdateBusinessShiftSettingsMutation } from '@/queries/team';
...
const shiftSettingsFormSchema = z.object({
  uploadNames: z.array(z.string().min(1, { message: 'Required' })).max(MAX_UPLOADS),
});
...
export default function BusinessShiftSettingsForm() {
  ...
  const updateShiftSettingsMutation = useUpdateBusinessShiftSettingsMutation();
  ...
  const addUploadableFile = debounce(() => {
    if (fields.length < MAX_UPLOADS) {
      ...
      notifications.show(`You can't add more than ${MAX_UPLOADS} uploadable files.`, {
        severity: 'error',
        autoHideDuration: 3000,
      });
    }
  ...
  return (
    <Container>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Typography variant="h6" sx={{ mb: 2 }}>Uploadable Files</Typography>
        
        <Stack direction="column" spacing={2} sx={{ mb: 3 }} maxWidth="sm">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    helperText={errors.uploadNames?.[index]?.message}
                  />
                )}
              />
              <IconButton
          ...
          {errors.uploadNames && typeof errors.uploadNames.message === 'string' && (
